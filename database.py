...
async def create_tables(conn):
    ...
    await conn.execute('''
                       ...
                       (
                           ...
                           chosen_worker BIGINT REFERENCES workers
                       (
                           worker_id
                       ),
                           media_type TEXT,
    ...
    await conn.execute('''
                       ...
                       (
                           ...
                           worker_id BIGINT REFERENCES workers
                       (
                           worker_id
                       )
                         ON DELETE CASCADE,
...
async def save_worker(conn, worker_id, data):
    await conn.execute('''
                       ...
                       VALUES ($1, $2, $3, $4, $5, $6, $7, $8) ON CONFLICT (worker_id) DO
                       UPDATE SET
...
async def save_pending_worker(conn, worker_id, data):
    await conn.execute('''
                       ...
                       VALUES ($1, $2, $3, $4, $5, $6, $7) ON CONFLICT (worker_id) DO
                       UPDATE SET
                           phone=$2, username=$3, name =$4,
...
async def save_offer(conn, order_id, worker_id, price=None, proposed_time=None):
    await conn.execute('''
                       ...
                       VALUES ($1, $2, COALESCE($3, 0), $4) ON CONFLICT (order_id, worker_id) DO
                       UPDATE SET
